...
ME0MuonTrackCollProducer::ME0MuonTrackCollProducer(const edm::ParameterSet& parset) :
	...
	muonQ(parset.getUntrackedParameter< bool >("muonQ")),
	parset_(parset)
{
...
void ME0MuonTrackCollProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	for(std::vector<reco::ME0Muon>::const_iterator ourmuon = OurMuons->begin();
			ourmuon != OurMuons->end(); ++ourmuon) {
		if(muonQ){
			if (!muon::isGoodMuon(me0Geom,*ourmuon, muon::Tight)) continue;}
		else{
			if (!muon::isGoodMuon(me0Geom,*ourmuon, muon::Loose)) continue;
		}
		reco::TrackRef trackref;    
		if (ourmuon->innerTrack().isNonnull()) trackref = ourmuon->innerTrack();
